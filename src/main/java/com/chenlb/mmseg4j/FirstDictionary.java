...
/**
 * words-first.dic词典类. 词库目录单例模式.<br/>
 * 保存单字与其频率,还有词库.<br/>
...
public class FirstDictionary {
	...
	private static final Logger log = ESLoggerFactory.getLogger(FirstDictionary.class.getName());
	...
	private static final ConcurrentHashMap<File, FirstDictionary> dics = new ConcurrentHashMap<File, FirstDictionary>();

	/**最大单词长度*/
	private static int  maxWordLen=1;//默认1为无效的长度
	
    /**
     * 词典的目录
     */
    private FirstDictionary(File path) {
        init(path);
    }
	 ...
	 * @see #getDefalutPath()
	 */
	public static FirstDictionary getInstance() {
	 ...
	 * @param path 词典的目录
	 */
	public static FirstDictionary getInstance(String path) {
	 ...
	 * @param path 词典的目录
	 */
	public static FirstDictionary getInstance(File path) {
		File normalizeDir = normalizeFile(path);
		FirstDictionary dic = dics.get(normalizeDir);
		if(dic == null) {
			dic = new FirstDictionary(normalizeDir);
	...
	/**
	 * @see FirstDictionary#clear(File)
	 */
	public static FirstDictionary clear(String path) {
	 ...
	 * @return 没有返回 null
	 */
	public static FirstDictionary clear(File path) {
		File normalizeDir = normalizeFile(path);
	...
	private Map<Character, CharNode> loadDic(File wordsPath) throws IOException {
		...
		InputStream wordsDicIn = this.getClass().getResourceAsStream("/data/words-first.dic");
		if(wordsDicIn != null) {
			File wordsDic = new File(this.getClass().getResource("/data/words-first.dic").getFile());
			loadWord(wordsDicIn, dic, wordsDic);
		}

		//加载自定义的words-first.dic
		File wordsFile = new File(wordsPath, "words-first.dic");;
	...
	private Map<Character, Object> loadUnit(File path) throws IOException {
		...
		if(unitFile.exists()) {
			...
			fin = FirstDictionary.class.getResourceAsStream("/data/units.dic");
			unitFile = new File(FirstDictionary.class.getResource("/data/units.dic").getFile());
		}
		...
		int lineNum = load(fin, new FileLoading() {
			...
			public void row(String line, int n) {
				...
				localUnit.put(line.charAt(0), FirstDictionary.class);
			}
		});
	 ...
	 * @return 文件总行数
	 */
	public int load(InputStream fin, FileLoading loading) throws IOException {
	...
	public synchronized boolean wordsFileIsChange() {
		...
		//检查是否有新文件
		File wordsFile = new File(dicPath, "words-first.dic");
	...
	public static File getDefalutPath() {
		if(defalutPath == null) {
			...
			if(defPath == null) {
				URL url = FirstDictionary.class.getClassLoader().getResource("data");
				if(url != null) {
					defPath = url.getFile();
